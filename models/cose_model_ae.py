        ...
        for batch_input, batch_target in iter(test_loader):
            if batch_num in [6,1800,1900,2000,2300]:
                ...
                with torch.no_grad():
                    ...
                    eval_loss, recon_chamfer = quantitative_ae_step(encoder_out, out_eval_parse_input, out_eval_parse_target, models_quant_eval, stats_tuple, eval_loss, self.device, self.config.rel_nhead)
                    # qualitative analysis
                    predicted_batch_stroke, predicted_batch_strat_pos, draw_seq_len = qualitative_ae_step(encoder_out, out_eval_parse_input, out_eval_parse_target, models_qual_eval, stats_tuple, self.device, self.config.rel_nhead, num_extra_pred = 5)
                    # obtain images
